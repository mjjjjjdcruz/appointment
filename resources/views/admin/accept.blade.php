            ...
            <div class="py-5 px-5 lg:flex-1 flex justify-between">
                ...
                <div class="text-right">
                    ...
                    <div class="flex justify-between">
                        <form action="/status/{{$appointment->appointment_id}}" method="POST">
                            @csrf
                            @method("PUT")
                            <input type="hidden" name="status" value="{{$appointment->status}}">
                            <input type="submit" name="submit" value="Accept" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-700 dark:hover:bg-blue-800 dark:focus:ring-blue-800 cursor-pointer"></input>
                        </form>
                        <form action="{{ route('status.returned', ['appointment_id' => $appointment->appointment_id]) }}" method="POST">
                            @csrf
                            @method("PUT")
                            <input type="hidden" name="status" value="{{$appointment->status}}">
                            <input type="submit" name="submit" value="Return" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-700 dark:hover:bg-blue-800 dark:focus:ring-blue-800 cursor-pointer"></input>
                        </form>
                    </div>
                </div>
            </div>
